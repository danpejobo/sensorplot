...
def configure_page() -> None:
    st.set_page_config(page_title="Sensorplot GUI", layout="wide", page_icon="📈")
...
def run_app() -> None:
    ...
    with st.sidebar:
        ...
        uploaded_files = st.file_uploader("Last opp filer (.xlsx / .csv)", accept_multiple_files=True)
        ...
        if uploaded_files:
            ...
            for uf in uploaded_files:
                ...
                with c2:
                    alias = st.text_input(uf.name, value=clean_name, key=f"alias_{uf.name}", label_visibility="collapsed")
        ...
        with st.expander("Avanserte kolonnenavn", expanded=False):
            ...
            if col_time and col_time.lower() == "none": col_time = None
    ...
    if file_registry:
        ...
        with c2:
            ...
            x_int = st.text_input("X-Akse Intervall (for PNG)", placeholder="Eks: 1M, 2W")

        if st.button("🚀 Generer Plott", type="primary", width="stretch"):
            results = calculate_series(formulas_input, file_registry, col_date, col_time, col_data, z_score)
            if results:
                st.session_state['sensor_results'] = results
                st.session_state['sensor_title'] = plot_title
                st.session_state['plot_id'] = st.session_state.get('plot_id', 0) + 1
                st.rerun()
    else:
    ...
    if 'sensor_results' in st.session_state:
        ...
        display_results_interface(st.session_state['sensor_results'], current_title, x_int)
...
def _process_single_line(line, file_registry, loaded_dfs, col_date, col_time, col_data, z_score):
    ...
    for alias in needed:
        ...
        if not already_loaded:
            try:
                ...
                # For å unngå race condition der to tråder laster samme fil samtidig, brukers lås rundt sjekk-og-last
                with cache_lock:
    ...
    try:
        ...
        for o in current_dfs[1:]:
            merged = pd.merge_asof(merged, o, on='Datetime', direction='nearest', tolerance=pd.Timedelta('10min'))
        
        safe_f = re.sub(r'\b([a-zA-Z0-9_\-æøåÆØÅ]+\.ch1)\b', r'`\1`', formula)
...
def calculate_series(formulas_text, file_registry, col_date, col_time, col_data, z_score):
    ...
    with st.spinner("Leser filer og beregner (Multithreaded)..."):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            # Start alle oppgaver
            futures = [
                executor.submit(_process_single_line, line, file_registry, loaded_dfs, col_date, col_time, col_data, z_score) 
                for line in lines
            ]
    ...
    # Vis alle feilmeldinger i hovedtråden (Streamlit UI er ikke trådsikkert)
    for err in errors:
        st.error(err)
    ...
    if raw_results:
        ...
        for r in raw_results:
            if r.label not in consolidated: consolidated[r.label] = []
        ...
        for lbl, dfs in consolidated.items():
            if len(dfs) == 1: 
                ...
                final_results.append(SensorResult(label=lbl, df=pd.concat(dfs).sort_values('Datetime')))
...
def display_results_interface(results, title, x_interval):
    ...
    if all_datetimes:
        ...
        if min_dt == max_dt:
            ...
            for res in results:
                mask = (res.df['Datetime'] >= start_filter) & (res.df['Datetime'] <= end_filter)
                filtered_df = res.df.loc[mask]
                if not filtered_df.empty:
                    filtered_results.append(SensorResult(label=res.label, df=filtered_df))
    ...
    with col_dl:
        png_buffer = generate_static_matplotlib(filtered_results, title, x_interval)
...
def generate_static_matplotlib(results, title, x_interval):
    ...
    for i, serie in enumerate(results):
        if not serie.df.empty:
            ...
            ax.plot(serie.df['Datetime'], serie.df['Resultat'], label=serie.label, color=farge, linewidth=1.5, alpha=0.9)
    ...
    if has_data:
        ...
        if x_interval:
            ...
            if match:
                ...
                if unit == 'D': locator = mdates.DayLocator(interval=num)
                elif unit == 'W': locator = mdates.WeekdayLocator(interval=num)
                elif unit == 'M': locator = mdates.MonthLocator(interval=num)
                elif unit == 'Y': locator = mdates.YearLocator(base=num)
        if not locator: locator = mdates.AutoDateLocator()
